...
global  chi beta mu sigma2  delta chi  mu Kbar a8 Aeq beq b N ii KK Nstar Kstar Lstar Hstar Sstar Cstar Ssstar Aastar aa ss Weeks scale C
...
H=2*(1-cdf('normal',K-Kbar,mu,sigma2))-1;
...
% 
% for i=2:length(a)
%     ahat(i)=(1-lambda)*ahat(i-1)+lambda*a(i-1);
%     shat(i)=(1-lambda)*shat(i-1)+lambda*s(i-1);
% end
% 
% C=12*((ahat+shat)/24-1);

%%%% Weekly weekend phase shift [2.5 2.5 2.5 2.5 3.5 3.5]
...
%% Zero out the homeostatic process
%H=zeros(size(a)); %% make H constant
%Hlag=zeros(size(a));  % make Hlag constant

%C=kron(ones(1,Weeks),[2.5 2.5 2.5 2.5 2.5 3.5 3.5])
%C=2.5*ones(1,Weeks*7);
U=sum(log(L)) + chi*sum(24/pi*(sin((a-C)*pi/12)-sin((s-C)*pi/12))      - H.*(2*s- a-24) + Hlag.*a);

% %% TGIF: Use CRRA for Saturday-Sunday.Code works, but results disappointing.
% gamma=.8;
% Party=[1;1;1;1;1;0;0]'; 
% %Party=[1;1;1;1;1;1;1]';
% PP=kron(ones(1,Weeks),Party);
% U=sum(PP.*log(L)+ (1-PP).*(L.^(1-gamma)-1)/(1-gamma)) + chi*sum(24/pi*(sin((a-C)*pi/12)-sin((s-C)*pi/12)) - H.*(2*s- a-24) + Hlag.*a);
...
%% TGIF: scale up leisure utility for Friday-Sunday.
 ...
 %% TGIF: Use CRRA with gamma<1 for F-Sunday, etc.
